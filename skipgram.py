...
# Create the batch,label pairs as (word, context).
skip_window_size = 5
train_data_words = [] # Each entry is for a word
train_data_context = [] # Each entry pais the word with a context
for sentence in data_words_coded:
    for pos,word in enumerate(sentence):
        ...
        train_data_context.extend(context) #Adds the context around the word
        train_data_words.extend([word] * len(context)) #Adds the word repeated many times
...
train_data_context = np.matrix(train_data_context).T #It's a matrix transpose to use as label
train_data_words = np.array(train_data_words) #It's a vector to use as batch
...
reverse_dictionary = dict(zip(context_pruned_dictionary.values(), context_pruned_dictionary.keys()))

f = open('output.txt','w')
for i in range(len(context_pruned_dictionary)):
